define(['MainCore'], function(ImagesInc_Core) {
        ...
        return {
            ...
            handelFavouritesClick: function () {
                ...
                try {
                    ...
                    favouritesPageObjDef = sandBox.getValueForKeyAsObjectFromStorage(ImagesInc_GlobalData.getFavouritesPageObjDefID());
                    if (!favouritesPageObjDef) {
                        favouritesPageObjDefID = ImagesInc_GlobalData.getFavouritesPageObjDefID();
                        // if Component definition is not in the storage, the page object definitions probably needs to be loaded
                        sandBox.loadPageDefinitionsFileAndCallBack(function () {
                            sandBox.getComponentObjAndCallback(favouritesPageObjDefID, sandBox.contextObj.buildFavouritesPage);
                        });
                    } else {
            ...
            buildFavouritesPage: function (favouritesPageObjDef) {
                if (favouritesPageObjDef && typeof favouritesPageObjDef === "object") {
                    ...
                    if (favouritesPageObjDef.cssFile && favouritesPageObjDef.cssPath) {
                        sandBox.loadCSSfileFromObjDef(favouritesPageObjDef.cssFile, favouritesPageObjDef.cssPath);
                    }
                    if (favouritedImagesArray && favouritedImagesArray.length > 0) {
                        ...
                        sandBox.logMessage(3, 'No favourites was found; from handelFavouritesClick ' + e.message);
                    }
                } else {
                    sandBox.logMessage(3, 'Page definition was not found, cannot render page; from handelFavouritesClick ' + e.message);
                    return false;
                }
            },
            updateContentPage: function (pageURL) {
                // if (pageURL === "/Modular-Programming-with-JavaScript/index.html") {
            ...
            buildIndexContentHTML: function (imagesArray) {
                ...
                for (var i = 0; i < imagesArray.length; i++) {
                    htmlStr += '<div class="imageDiv">' +
                        '<a href="/' + imagesArray[i] + '" ><img src="Images/' +
                        imagesArray[i] + '" alt="' + imagesArray[i] + '" data-name="' + imagesArray[i] + '" /></a>' +
                        '<div class="imageIndexTextDiv">' + this.getAnchorHTMLStr(
                            imagesArray[i], this.checkFavouriteCookie(imagesArray[i])) + '</div></div>';
                }
            ...
            buildImagePageHTML: function (imgName) {
                var htmlStr;
                htmlStr =
                    '<div class="imagePageMainDiv"><img src="Images/' +
                    imgName + '" alt="' + imgName + '" data-name="' + imgName + '" /></div>';
                return htmlStr;
            },
            getAnchorHTMLStr: function (id, favourited) {
                var htmlStr;
                if (!favourited) {
                    // htmlStr = '<a href="' + '"' + ' id="' + id + '">Add to Favorites</a>';
                    htmlStr = '<a href="" id="' + id + '">Add to Favorites</a>';
                } else {
                    // htmlStr = '<a href="' + '"' + ' id="' + id + '" data-state="favourited" ' + '>Remove from Favorites</a>';
                    htmlStr = '<a href="" id="' + id + '" data-state="favourited">Remove from Favorites</a>';
                }
                return htmlStr;
            },
    ...
    ImagesInc_Core.registerComponent("mainPageContainer", "content", callback)
});
